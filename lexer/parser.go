...
func NewBasicParser() BasicParser {
	...
	expr0 := Rule()
	primary := RuleByType(NewPrimaryExpr([]TreeNode{})).Or([]Parser{Rule().Sep("(").Ast(expr0).Sep(")"),
		Rule().Number(NewNumberNode(nil)),
		Rule().Identifier(NewVariableNode(nil), reserved),
		Rule().String(NewStringNode(nil))})
	factor := Rule().Or([]Parser{RuleByType(NewNegativeExprNode([]TreeNode{})).Sep("-").Ast(primary), primary})
	...
	program := Rule().Or([]Parser{statement, RuleByType(NewNullStatementNode([]TreeNode{}))}).Sep(";", EOL)
	return BasicParser{
		reserved:   reserved,
		operators:  operators,
		parser:     expr0,
